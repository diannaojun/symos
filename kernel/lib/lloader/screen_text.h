...
inline static char* ins_scrn_txt_get_addr_c(struct INS_SCRN_TXT* ts,u16 x,u16 y) {
    x%=ts->w,y%=ts->h;
    return (char*)(ts->vm+((y*ts->w+x)<<1));
}
inline static u8* ins_scrn_txt_get_addr_s(struct INS_SCRN_TXT* ts,u16 x,u16 y) {
    return (u8*)(ins_scrn_txt_get_addr_c(ts,x,y)+1);
}
void ins_scrn_txt_swp(u8 id) {
    return ;
}
void ins_scrn_txt_clr(struct INS_SCRN_TXT* ts) {
    memset(ts->vm,0,(size_t)(ts->h*ts->w)<<1);
    return ;
}
void ins_scrn_txt_scr(struct INS_SCRN_TXT* ts,u16 lines) {
    static size_t i,e,s,k;
    s=(size_t)ts->w<<1,k=s*lines;
    e=(size_t)ts->vm+(ts->h-1)*k;
    for(i=(size_t)ts->vm; i<e; i+=s)
...
void ins_scrn_txt_putc(struct INS_SCRN_TXT* ts,char c) {
    ...
    default:
        *ins_scrn_txt_get_addr_c(ts,ts->cx,ts->cy)=c;
        *ins_scrn_txt_get_addr_s(ts,ts->cx,ts->cy)=ts->style;
        ts->cx++;break;
    }
    if(ts->cx>=ts->w) {
        ts->cx%=ts->w;
